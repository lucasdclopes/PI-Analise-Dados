...
public class Normalizador {
	...
	private BigDecimal normalizar(BigDecimal ponto, BigDecimal min, BigDecimal max) {
		// (valor - menorValor) / (maiorValor - menorValor)
		return ( ponto.subtract(min) ).divide((max.subtract(min)),4,RoundingMode.HALF_UP); 
	}
	//normaliza todos os itens. Co2. Pib e Pib per Capita
	public List<PibCo2DadosDto> normalizarPibCo2(List<PibCo2DadosDto> entrada) {
		...
		entrada.forEach(e -> {
			...
			dadosNormalizados.add(new PibCo2DadosDto( 
				e.ano(), 
				new TendenciaCentralDto(
						pib.tendenciaCentral(),pib.min(),pib.max(),
						normalizar(
								pib.tendenciaCentral(),minPib,maxPib
								)
						), 
				new TendenciaCentralDto(
						pibPerCap.tendenciaCentral(),pibPerCap.min(),pibPerCap.max(),
						normalizar(
								pibPerCap.tendenciaCentral(),minPibPerCap,maxPibPerCap
								)
						), 
				new TendenciaCentralDto(
						co2.tendenciaCentral(),co2.min(),co2.max(),
						normalizar(
								co2.tendenciaCentral(),minCo2,maxCo2
								)
						)
				));
		});
	...
	public BigDecimal getMinPibCo2( 	//Menor valor da coleção
			List<PibCo2DadosDto> entrada,Function<? super PibCo2DadosDto, ? extends BigDecimal> mapper 
			) {
		try {
	...
	public BigDecimal getMaxPibCo2( 	//Maior valor da coleção
			List<PibCo2DadosDto> entrada,Function<? super PibCo2DadosDto, ? extends BigDecimal> mapper 
			) {
		try {
